...
(() => {
  ...
  const ROLES = {
    sekreter: { title: 'The postal captain', description: 'A brass-buttoned captain in a teal uniform and peaked cap, stamping and routing every letter from a red leather satchel.', skin: C.skin2, hair: '#4a3226', coat: C.teal2, trim: C.gold2, pants: '#2b3a44', boots: C.wood0, work: { n: 8, fps: 8 } },
  ...
  function torso(q, s, y, role) {
    ...
    switch (role) {
      case 'sekreter':
        for (const yy of [-17, -14, -11]) { q.px(-1, yy + Y, C.gold3); q.px(2, yy + Y, C.gold3); }
        q.rect(x, -10 + Y, w, 1, C.wood0); q.px(0, -10 + Y, C.gold3);
        q.rect(x - 1, -20 + Y, 3, 2, C.gold2); q.rect(x + w - 2, -20 + Y, 3, 2, C.gold2); q.px(x - 1, -20 + Y, C.gold4);
        q.line(4, -20 + Y, -5, -10 + Y, '#9a3a2a', 1); q.line(5, -20 + Y, -4, -10 + Y, '#6e2a20', 1);
        q.rect(-10, -12 + Y, 6, 6, '#9a3a2a'); q.rect(-10, -12 + Y, 6, 2, '#b84e3a'); q.px(-7, -10 + Y, C.gold3); q.rect(-10, -7 + Y, 6, 1, '#6e2a20');
        break;
      case 'girard':
  ...
  function head(q, s, y, role, eyes = 'open', mouth = 'smile', look = 0) {
    ...
    if (mouth === 'smile') { q.px(2, hy + 9, '#8a3a30'); q.rect(3, hy + 10, 2, 1, '#8a3a30'); q.px(5, hy + 9, '#8a3a30'); }
    else if (mouth === 'flat') { q.rect(3, hy + 10, 2, 1, '#8a3a30'); }
    else if (mouth === 'o') { q.rect(3, hy + 9, 2, 2, '#5a1e1a'); }
    else if (mouth === 'frown') { q.px(3, hy + 10, '#8a3a30'); q.px(4, hy + 10, '#8a3a30'); q.px(2, hy + 11, '#8a3a30'); }
    else if (mouth === 'grin') { q.rect(2, hy + 9, 4, 2, C.white); q.rect(2, hy + 10, 4, 1, '#8a3a30'); }
    const brow = (col, h = 1) => { q.rect(ex - 1, hy + 4, 2, h, col); q.rect(ex + 3, hy + 4, 2, h, col); };
    switch (role) {
      case 'sekreter':
        q.rect(-6, hy + 1, 4, 6, hair); q.rect(-6, hy + 1, 13, 2, hair); brow(hair);
        q.rect(-7, hy - 3, 14, 4, C.teal2); q.rect(-6, hy - 4, 12, 1, C.teal3); q.rect(-7, hy - 3, 1, 4, C.teal3); q.rect(5, hy - 3, 2, 4, C.teal1);
        q.rect(-7, hy, 14, 2, C.gold2); q.rect(-7, hy, 14, 1, C.gold3); q.rect(1, hy + 2, 8, 1, C.teal0); q.rect(2, hy + 2, 6, 1, '#16302e'); q.px(1, hy - 2, C.gold3); q.px(2, hy - 2, C.gold4);
        break;
      case 'girard':
  ...
  function pose(role, anim, f) {
    ...
    const rest = { sekreter: q => envelope(q, p.fh[0] + 1, p.fh[1] - 1), girard: q => coin(q, p.fh[0], p.fh[1] - 2, 0), 'text-writer': q => quill(q, p.fh[0], p.fh[1]), gazeteci: q => notepad(q, p.fh[0] + 1, p.fh[1] - 1), bayes: q => { q.rect(p.fh[0], p.fh[1] - 12, 2, 13, C.gold1); q.px(p.fh[0], p.fh[1] - 12, C.gold4); q.rect(p.fh[0] - 1, p.fh[1] - 13, 4, 2, C.gold2); }, kandinsky: q => brush(q, p.fh[0], p.fh[1], 2, -6, '#9a5fac'), kole: q => hammer(q, p.fh[0], p.fh[1] + 2, -1.9), 'scum-master': q => ladle(q, p.fh[0], p.fh[1]) };
    ...
    switch (role) {
      case 'sekreter': {
        ...
        p.bh = [8, -13]; p.fh = ups; p.dy = f === 2 || f === 3 ? 1 : 0; p.mouth = f === 2 ? 'grin' : 'smile'; p.eyes = f === 2 ? 'happy' : 'open';
        p.item = q => {
  ...
  function drawLead(q, role, anim, f) {
    ...
    const sleeve = S(s.coat, -.12), hand = s.skin;
  ...
  function stateRing(k, x, y, t, state) {
    if (state === 'off') return;
    const col = C[state] || C.idle;
    if (state === 'waiting' || state === 'error') {
      const sp = state === 'error' ? 1.6 : 1, q = (t * sp) % 1;
      k.alpha(1 - q, () => k.ring(x, y, 12 + q * 10, 4 + q * 4, col));
      k.alpha(.35, () => k.ellipse(x, y, 12, 4, col));
      k.ring(x, y, 12, 4, col);
    } else if (state === 'working') {
      k.alpha(.25, () => k.ellipse(x, y, 12, 4, col));
      const n = 40, h = mod(Math.floor(t * 14), n);
      for (let i = 0; i < n; i++) { const a = i / n * Math.PI * 2, d = (i - h + n) % n; if (d > 8 && d % 5) continue; k.px(x + Math.cos(a) * 12, y + Math.sin(a) * 4, d <= 8 ? C.white : col); }
    } else k.alpha(.55, () => k.ring(x, y, 12, 4, col));
  }

  /* ---------- Public: draw a lead ---------- */
  function lead(k, role, x, y, t, state = 'idle', opts = {}) {
    if (!ROLES[role]) return;
    const fr = frameFor(role, state, t, opts);
  ...
  function portrait(canvas, role, state = 'working', t = 1.2) {
    ...
    k.ellipse(0, 1, 14, 4, '#00000022'); lead(k, role, 0, 0, t, state, { indicator: true }); c.restore();
  }
