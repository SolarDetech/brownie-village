...
(() => {
  ...
  // The HQ dog: press and hold on it, then move the mouse over it to pet it. Or pick up its ball, shake it, throw it.
  function pet(e) {
    const w = toWorld(e), h = window.HQDog?.hit(...w); if (!h) return false;
    viewport.focus({ preventScroll: true }); pointers.set(e.pointerId, { x: e.clientX, y: e.clientY }); viewport.setPointerCapture(e.pointerId);
    gesture = { dog: h.d, ball: h.part === 'ball', id: e.pointerId }; viewport.classList.add('petting'); hideWelcome();
    if (gesture.ball) { HQDog.pickBall(h.d, ...w); say('You picked up the ball. Shake it to call the dog.'); }
    else { HQDog.grab(h.d, w[0]); say('Petting the Bernese mountain dog.'); }
    return true;
  }
  function dropAvatar() {
  ...
  viewport.addEventListener('pointermove', e => { if (pointers.size) return; const w = toWorld(e), dog = !!window.HQDog?.hit(...w); window.HQDog?.cursor(...w); viewport.classList.toggle('can-pet', dog); viewport.classList.toggle('can-carry', !dog && !!Avatars.hit(...w)); });
  viewport.addEventListener('pointerdown', e => { if (e.target.closest('button')) return; if (gesture?.avatar || gesture?.dog) return; if (!pointers.size && (pet(e) || carry(e))) return; viewport.focus({ preventScroll: true }); pointers.set(e.pointerId, { x: e.clientX, y: e.clientY }); viewport.setPointerCapture(e.pointerId); gesture = { x: e.clientX, y: e.clientY, startX: e.clientX, startY: e.clientY, moved: false }; if (pointers.size === 2) { const [a, b] = [...pointers.values()]; pinch = Math.hypot(a.x - b.x, a.y - b.y); } viewport.classList.add('dragging'); });
  viewport.addEventListener('pointermove', e => { if (!pointers.has(e.pointerId)) return; pointers.set(e.pointerId, { x: e.clientX, y: e.clientY }); if (gesture?.avatar) { Avatars.move(gesture.avatar, ...toWorld(e)); return; } if (gesture?.dog) { const w = toWorld(e); HQDog.cursor(...w); if (!gesture.ball) HQDog.stroke(gesture.dog, ...w); else if (HQDog.moveBall(gesture.dog, ...w)) say('The dog is waiting for the throw.'); return; } if (pointers.size === 2) { const [a, b] = [...pointers.values()], d = Math.hypot(a.x - b.x, a.y - b.y); if (pinch) zoom(d / pinch, (a.x + b.x) / 2, (a.y + b.y) / 2); pinch = d; gesture.moved = true; return; } if (!gesture) return; camera.x += e.clientX - gesture.x; camera.y += e.clientY - gesture.y; if (Math.hypot(e.clientX - gesture.startX, e.clientY - gesture.startY) > 4) gesture.moved = true; gesture.x = e.clientX; gesture.y = e.clientY; viewMode = 'custom'; clampCamera(); positionLabels(); hideWelcome(); });
  function end(e) { if (!pointers.has(e.pointerId)) return; if (gesture?.avatar) { pointers.delete(e.pointerId); dropAvatar(); gesture = null; return; } if (gesture?.dog) { pointers.delete(e.pointerId); if (gesture.ball) say(HQDog.throwBall(gesture.dog) ? 'Fetch!' : 'You dropped the ball.'); else HQDog.release(gesture.dog); viewport.classList.remove('petting'); gesture = null; return; } const wasPinch = !!pinch; pointers.delete(e.pointerId); if (gesture && !gesture.moved && !wasPinch && e.type === 'pointerup') { const r = viewport.getBoundingClientRect(), x = (e.clientX - r.left - camera.x) / camera.scale, y = (e.clientY - r.top - camera.y) / camera.scale, z = world.hitTest(x, y); if (z) visit(z, viewport); } if (pointers.size === 1) { const p = [...pointers.values()][0]; gesture = { x: p.x, y: p.y, startX: p.x, startY: p.y, moved: true }; } else gesture = null; pinch = null; if (!pointers.size) viewport.classList.remove('dragging'); }
