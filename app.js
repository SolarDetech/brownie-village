...
(() => {
  ...
  // The HQ dog: press and hold on it, then move the mouse over it to pet it.
  function pet(e) {
    const dog = window.HQDog?.hit(...toWorld(e)); if (!dog) return false;
    viewport.focus({ preventScroll: true }); pointers.set(e.pointerId, { x: e.clientX, y: e.clientY }); viewport.setPointerCapture(e.pointerId);
    gesture = { dog, id: e.pointerId }; HQDog.grab(dog, toWorld(e)[0]); viewport.classList.add('petting'); hideWelcome(); say('Petting the Bernese mountain dog.'); return true;
  }
  function dropAvatar() {
  ...
  viewport.addEventListener('pointermove', e => { if (pointers.size) return; const w = toWorld(e), dog = !!window.HQDog?.hit(...w); viewport.classList.toggle('can-pet', dog); viewport.classList.toggle('can-carry', !dog && !!Avatars.hit(...w)); });
